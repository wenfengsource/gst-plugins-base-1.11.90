...
#define MAX_READ_SIZE                   1358   // wenfeng
...
case PROP_LOCAL_PORT:
      tcpclientsrc->localport = g_value_get_int (value);
      break;
case PROP_LOCAL_IP:
	 if (!g_value_get_string (value)) {
        g_warning ("host property cannot be NULL");
      ...
      g_free (tcpclientsrc->localip);
      tcpclientsrc->localip = g_strdup (g_value_get_string (value));
      break;
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TIMEOUT:
      ...
      break;
   case PROP_LOCAL_IP:
      g_value_set_string (value, tcpclientsrc->localip);
      break;
    case PROP_LOCAL_PORT:
      g_value_set_int (value, tcpclientsrc->localport);
      break;
...
gst_tcp_client_src_start (GstBaseSrc * bsrc)
{
  ...
  if (!src->socket)
    goto no_socket;
  	...
  	GST_DEBUG_OBJECT (src,"src->localip = %s \n", src->localip);
 	GST_DEBUG_OBJECT (src,"src->localport = %d \n", src->localport);
     if(src->localport != 0)
	  {
		int flag= g_socket_bind(src->socket,
		  G_SOCKET_ADDRESS(g_inet_socket_address_new(g_inet_address_new_from_string(src->localip) ,src->localport)),   // wenfeng
		  TRUE,
		  &err);
		GST_DEBUG_OBJECT (src,"socket bind ----flag = %d\n", flag);
		if(flag == FALSE)
		{
  ...
  /* connect to server */
// connect 4 times vvv wenfeng
   int count =4;
	while(count--)
	{
	  if (!g_socket_connect (src->socket, saddr, src->cancellable, &err))
		{
			printf("connect tcp sink server error %d\n", count);
				if(count == 1)		
			goto connect_failed;
