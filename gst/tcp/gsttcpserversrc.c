...
#define MAX_READ_SIZE                    1316    // wenfeng
...
// vvv wenfeng
 g_object_class_install_property (gobject_class, TIME_OUT,
      g_param_spec_int ("timeout", "Timeout",
          "Tcp Server waiting client connection timeout",
          0, 100, 30,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
// ^^^ wenfeng
  /**
...
// vvv wenfeng
int timeout_check(gpointer ptr)
{
	GstTCPServerSrc *src;
	src = (GstTCPServerSrc *) ptr;
	if(src->client_socket == NULL)
    {
		        gst_element_post_message (GST_ELEMENT_CAST (src),
            gst_message_new_element (GST_OBJECT_CAST (src),
                gst_structure_new ("tcpserversrctimeout",
                    "timeout", G_TYPE_UINT64, src->timeout, NULL)));

 		GST_DEBUG_OBJECT (src, "waitting tcp client timeout");
	}
...
gst_tcp_server_src_create (GstPushSrc * psrc, GstBuffer ** outbuf)
{
  ...
  // wenfeng  add timeout event if no have client connection

  //  printf("==333==% lld \n", src->timeout);
	//g_timeout_add_seconds(src->timeout,timeout_check ,src);
    if(src->timeout != 0)
	{
		//src->timeout = src->timeout / 1000000000;
		//printf("====% lld \n", src->timeout);
    	g_socket_set_timeout(src->server_socket,src->timeout) ;
	}
    src->client_socket =
  ...
  //  GST_DEBUG_OBJECT (src, "closing server socket");
	 if(src->timeout != 0)
     g_socket_set_timeout(src->client_socket,src->timeout) ;
...
// vvv wenfeng
else if ((condition & G_IO_ERROR_TIMED_OUT)) {
      GST_DEBUG_OBJECT (src, "timeout");
    //  *outbuf = NULL;
    //  ret = GST_FLOW_EOS;
     // goto done;
    }
// ^^^ wenfeng
  ...
  if (avail > 0) {
 //  vvv wenfeng
	if(src->getdata_flag == 0)
	{
			src->getdata_flag = 1;
		        gst_element_post_message (GST_ELEMENT_CAST (src),
            gst_message_new_element (GST_OBJECT_CAST (src),
                gst_structure_new ("tcpserversrcgetdata",
                    "getdata", G_TYPE_UINT64, avail, NULL)));
	}
 // ^^^ wenfeng
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
	...
	// vvv wenfeng
	case TIME_OUT:
	tcpserversrc->timeout = g_value_get_int (value);
      break;
	// ^^^ wenfeng
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
	...
	// vvv wenfeng
 case TIME_OUT:
      g_value_set_int (value, g_atomic_int_get (&tcpserversrc->timeout));
      break;
	// ^^^ wenfeng
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
